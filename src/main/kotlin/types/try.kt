...
public trait Try<A : Any> : Functor<A> {
    ...
    override fun fmap<B : Any>(fn: (A) -> B): Try<B>
    fun bind<B : Any>(fn: (A) -> Try<B>): Try<B>
}

public data class Success<A : Any>(val value: A) : Try<A> {
    override fun <B : Any> fmap(fn: (A) -> B): Success<B> = Success(fn(value))
...
public data class Failure<A : Any>(val value: Throwable) : Try<A> {
    ...
    override fun <B : Any> fmap(fn: (A) -> B): Failure<B> = this as Failure<B>
